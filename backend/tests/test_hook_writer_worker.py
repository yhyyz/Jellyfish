"""W12-T4: ``hook_writer`` worker 执行器测试。
...
测试策略（与 W5-T3 ``test_compliance_check_worker.py`` 同构）：
    ...
    - ``langchain_openai.ChatOpenAI`` 在 worker 路径上 monkeypatch 成
      返回固定 :class:`ShotHook` JSON 的 ``BaseChatModel``，避免触网；
    - **不**替换 :class:`HookWriterAgent`：本任务的集成价值就是 worker
      正确把 :class:`HookWriteVars` 喂给 Agent，并把回写到 variant 上的
      字段限制在 ``opening_hook``，mock 掉就失去意义；
...
# ---------------------------------------------------------------------------
# 共用 LLM mock & ShotHook JSON 构造工具
# ---------------------------------------------------------------------------
...
class _MockChatModel(BaseChatModel):
    ...
    把这个类塞进去，于是 ``build_default_text_llm_sync`` 拿到的就是该桩。
    """
    ...
    def __init__(self, response: str = "", **_kwargs: Any) -> None:
        super().__init__()
        # ``BaseChatModel`` 走 pydantic，需要绕过字段保护塞 response。
...
def _hook_json(
    *,
    hook_text: str = "你以为这只是普通保湿吗？",
    pattern_id: str = "conflict_001",
    pattern_type: str = "conflict",
    rationale: str | None = "用反差制造认知冲突",
) -> str:
    """构造一个合法的 :class:`ShotHook` JSON 字符串供 mock LLM 返回。"""
...
_DEFAULT_BREAKDOWN: dict[str, Any] = {
    "opening_hook": "原始钩子，应被改写",
    "cta_text": "原始 CTA，应保持不变",
...
async def session_factory(tmp_path):
    ...
    file-backed 是必须的：worker 内部多次 ``async_session_maker()``，
    多 connection 的 ``:memory:`` 会拿到独立内存库，破坏断言。
    """
...
def _base_run_args(**overrides: Any) -> dict[str, Any]:
    ...
    args: dict[str, Any] = {
        ...
        "product_description": "提亮+抗氧化",
        "audience_pain_points": ["暗沉", "毛孔粗大"],
        "audience_demographics": {"age_range": "25-35"},
    }
...
async def test_runner_validates_required_variant_id(
    ...
    """``variant_id`` 缺失/为空 → task 标记 failed。"""
    ...
    _patch_chat_openai(monkeypatch, _MockChatModel(_hook_json()))
    ...
    with pytest.raises(ValueError, match="variant_id"):
        await run_hook_writer_task(task_id, run_args)
...
# ---------------------------------------------------------------------------
# 2. 持久化：原地 patch opening_hook
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_runner_patches_variant_opening_hook(
    session_factory: Any, monkeypatch: pytest.MonkeyPatch
) -> None:
    """LLM 返回的 hook_text 必须落到 variant.script_breakdown['opening_hook']。"""
    ...
    _patch_chat_openai(
        monkeypatch,
        _MockChatModel(_hook_json(hook_text="新的钩子文本")),
    )
    ...
    assert breakdown.get("opening_hook") == "新的钩子文本"
...
@pytest.mark.asyncio
async def test_runner_returns_hook_text_in_result(
    session_factory: Any, monkeypatch: pytest.MonkeyPatch
) -> None:
    """task.result 必须暴露 hook_text + pattern 元数据 + variant_id。"""
    ...
    assert result.get("hook_text") == "新的钩子文本"
    assert result.get("pattern_id") == "conflict_001"
    assert result.get("pattern_type") == "conflict"
    assert result.get("rationale") == "对比刺激"
...
async def test_runner_marks_failed_on_agent_error(
    ...
    _patch_session_maker(monkeypatch, session_local)
    # 无效响应：非 JSON、无法被 json-repair 救回
    _patch_chat_openai(monkeypatch, _MockChatModel("this is not json at all"))

    run_args = _base_run_args()
    task_id = await _create_task(session_local, run_args=run_args)
    with pytest.raises(Exception):  # noqa: B017 — 捕获原始 ValidationError
        await run_hook_writer_task(task_id, run_args)
...
def test_executor_registered_with_correct_task_kind() -> None:
    """task_executor_registry 解析 ``hook_writer`` → AbstractAsyncDelegatingExecutor。"""
...
def test_executor_timeout_set_to_180s() -> None:
    ...
    executor = build_hook_writer_executor()
    assert executor.timeout_seconds == DEFAULT_TIMEOUT_SEC == 180.0
    assert executor.task_kind == TASK_KIND


# ---------------------------------------------------------------------------
# 7. 仅改 opening_hook，不动其它字段
# ---------------------------------------------------------------------------
...
async def test_runner_preserves_other_breakdown_fields(
    ...
    """patch 仅更新 opening_hook；cta_text / shots / total_shots 必须保持原值。"""
    ...
    _patch_session_maker(monkeypatch, session_local)
    _patch_chat_openai(
        monkeypatch, _MockChatModel(_hook_json(hook_text="REWRITTEN"))
    )
    ...
    assert variant is not None
    breakdown = variant.script_breakdown or {}
    assert breakdown["opening_hook"] == "REWRITTEN"
    # 其他字段一字不改
    assert breakdown["cta_text"] == "ORIGINAL CTA"
...
async def test_runner_passes_full_vars_to_agent(
    ...
    """run_args 中的全部字段必须以 ``HookWriteVars`` 形式透传给 Agent。"""
    ...
    # pylint: disable=import-outside-toplevel
    from app.services.commerce import hook_writer_worker as worker_mod

    real_a_write_hook = worker_mod.HookWriterAgent.a_write_hook  # type: ignore[attr-defined]

    async def _spy_a_write_hook(  # noqa: ANN001,A002
        self: Any,
        *,
        vars: Any,  # pylint: disable=redefined-builtin
    ):
        captured["vars"] = vars
        return await real_a_write_hook(self, vars=vars)

    monkeypatch.setattr(
        "app.services.commerce.hook_writer_worker.HookWriterAgent.a_write_hook",
        _spy_a_write_hook,
    )

    run_args = _base_run_args(
        product_description="高浓度 VC，温和不刺激",
        audience_pain_points=["暗沉", "毛孔"],
        audience_demographics={"age_range": "25-35", "gender": "female"},
    )
    task_id = await _create_task(session_local, run_args=run_args)
    await run_hook_writer_task(task_id, run_args)
    ...
    assert captured_vars.pattern_id == "conflict_001"
    assert captured_vars.pattern_type == "conflict"
    assert captured_vars.product_name == "Vitamin C Serum"
    assert captured_vars.product_description == "高浓度 VC，温和不刺激"
    assert captured_vars.audience_pain_points == ["暗沉", "毛孔"]
    assert captured_vars.audience_demographics == {
        "age_range": "25-35",
        "gender": "female",
    }
