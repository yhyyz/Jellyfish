...
class VoiceRegion(str, Enum):
    """DashScope 区域端点枚举（P5 W29 引入）。

    与 ``VoicePack.region`` 列绑定，决定 voice clone 调用走北京还是新加坡：

    - ``cn_beijing``：``dashscope.aliyuncs.com``，``cosyvoice-v3.5-plus`` 仅
      在该地域可用，是中文音色的默认选择。
    - ``ap_singapore``：``dashscope-intl.aliyuncs.com``，海外音色（en/ja/ko
      等）首选，避免合规风险。

    同一 voice_id 不能跨区使用，因此该列必须随 voice_id 一并持久化。
    """
...
class AudioStrategy(str, Enum):
    ...
    silent_with_tts = "silent_with_tts"
    keep_native = "keep_native"


class ProductFocusLevel(str, Enum):
